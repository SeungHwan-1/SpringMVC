...
public class ProblemOfUsingDirectSQLTests {
     ...
     * 코드의 품질을 개선하고 유지 보수성을 높이기 위해 사용한다. 메소드 단위로 테스트를 수행하며,
     ...
     * 1. 데이터 변환, SQL 작성, JDNC API 코드 등의 중복 작성(개발시간 증가, 유지 보수성 저하
     * 2. SQL 에 의존하여 개발
     * 3. 패러다임 불일치
     * 4. 동일성 보장 문제
     */
    ...
    //2. SQL 에 의존하여 개발
    /*
     * 요구사항 변겨에 따라 어플리케이션 수정이 SQL 의 수정으로도 이어진다.
    ...
    @Test
    void testChangeSectColumns() throws SQLException {
    ...
    void testAssociatedObject() throws SQLException {
        ...
        Statement stmt = con.createStatement();
        ResultSet rset = stmt.executeQuery(query);
        List<MenuAndCategory> menuList = new ArrayList<>();
    ...
    while(rset.next())
    {
        ...
        menu.setMenuName(rset.getString("MENU_NAME"));
        menu.setMenuPrice(rset.getInt("MENU_PRICE"));
        menu.setCategory(new Category(rset.getString("CATEGORY_NAME"),
    ...
    void testEquals() throws SQLException {
        ...
        while (rset.next()){
            menu1 = new Menu();
            menu1.setCategoryCode(rset.getInt("MENU_CODE"));
        ...
        while (rset2.next()){
            menu2 = new Menu();
            menu2.setCategoryCode(rset2.getInt("MENU_CODE"));
