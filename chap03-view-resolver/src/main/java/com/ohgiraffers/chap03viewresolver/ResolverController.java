...
public class ResolverController {
    ...
    public String stringRedirect(Model model) {
        return "redirect:/"; //스트링에서  리다이렉트는 리다이렉트 콜론 슬래쉬
        //기본은 포워드같은 느낌 //접두사로 redirect:를 하면 리다이렉트시킨다.
    }
    @GetMapping("string-redirect-attr")
    public String stringRedirectFlashAttributes(RedirectAttributes rttr) {
        /*
         ...
         * 클라이언트는 리다이렉트된 URL로 GET요청을 보내고 그결과를 화면에 표시한다.
         * 이렇게되면 이후 새로고침시에도 GET요청을 보내기 때문에 중복 데이터 처리가 발생하지 않는다.
         */

        /*
         * ModelAndView - spring 에서 모델과 뷰를 함께처리하기위한 클래스.
         */
    }
